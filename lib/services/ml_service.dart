...
class MLService {
  static List<Anomaly> detectAnomalies(List<TimeEntry> entries) {
    ...
    for (final entry in dailyHours.entries) {
      ...
      if (hours > stats.avgDailyHours * 2) {
        anomalies.add(Anomaly(
          type: AnomalyType.unusualHours,
          severity: hours > stats.avgDailyHours * 3 ? 'high' : 'medium',
          message: 'Worked ${hours.toStringAsFixed(1)}h on ${_formatDate(entry.key)} - ${((hours / stats.avgDailyHours - 1) * 100).toInt()}% above your average',
          date: entry.key,
        ));
      }
    }
    ...
    for (final e in entries.where((e) => e.startTime != null)) {
      ...
      if (hour >= 0 && hour < 5) {
        anomalies.add(Anomaly(
          ...
          severity: 'medium',
          message: 'Logged ${e.totalTime.toStringAsFixed(1)}h at $hour:${e.startTime!.minute.toString().padLeft(2, '0')} - unusual time for work',
          date: e.date,
        ));
      }
    }
    ...
    for (int i = 0; i < sortedDates.length - 1; i++) {
      ...
      if (gap > 5) {
        anomalies.add(Anomaly(
          ...
          severity: gap > 10 ? 'high' : 'low',
          message: '$gap days without work logged - longest gap in recent history',
          date: sortedDates[i],
        ));
      }
    }
    ...
    if (weekendHours > 0 && weekdayHours > 0) {
      ...
      if (ratio > 0.3) {
        anomalies.add(Anomaly(
          ...
          severity: 'medium',
          message: '${weekendHours.toStringAsFixed(1)}h weekend work - ${(ratio * 100).toInt()}% of weekday hours',
          date: DateTime.now(),
        ));
      }
    }
  ...
  static List<Pattern> recognizePatterns(List<TimeEntry> entries) {
    ...
    for (final seq in sequences.entries) {
      if (seq.value > 2) {
        patterns.add(Pattern(
          ...
          description: 'You often work on "${seq.key.second}" after "${seq.key.first}"',
    ...
    final avgHours = (dayStats.values.fold(0.0, (a, b) => a + b) / dayStats.length).toDouble();
    final bestDay = dayStats.entries.reduce((a, b) => a.value > b.value ? a : b);
    final worstDay = dayStats.entries.reduce((a, b) => a.value < b.value ? a : b);
    
    if (bestDay.value > avgHours * 1.3) {
      patterns.add(Pattern(
        type: PatternType.dayOfWeek,
        description: 'Your productivity peaks on ${_dayName(bestDay.key)} (${((bestDay.value / avgHours - 1) * 100).toInt()}% above average)',
        confidence: 0.8,
        impact: 'positive',
      ));
    }
    ...
    if (worstDay.value < avgHours * 0.7) {
      patterns.add(Pattern(
        type: PatternType.dayOfWeek,
        description: 'Productivity drops ${((1 - worstDay.value / avgHours) * 100).toInt()}% on ${_dayName(worstDay.key)}',
        confidence: 0.75,
        impact: 'negative',
      ));
    }
    ...
    if (switchRate > 0.5) {
      patterns.add(Pattern(
        ...
        description: 'High task switching detected - you switch projects ${(switchRate * 100).toInt()}% of the time',
        confidence: 0.85,
        impact: 'negative',
      ));
    }
  ...
  static List<Recommendation> generateRecommendations(List<TimeEntry> entries) {
    ...
    if (stats.avgProjectsPerDay > 4) {
      recommendations.add(Recommendation(
        type: RecommendationType.focus,
        title: 'Reduce Context Switching',
        description: 'You work on ${stats.avgProjectsPerDay.toStringAsFixed(1)} projects/day. Try limiting to 3-4 for better focus.',
        priority: 'high',
    ...
    if (avgSessionLength > 4) {
      recommendations.add(Recommendation(
        ...
        description: 'Your average session is ${avgSessionLength.toStringAsFixed(1)}h. Consider 5-10min breaks every 90min.',
        priority: 'medium',
    ...
    if (weekendRatio > 0.25) {
      recommendations.add(Recommendation(
        ...
        description: '${(weekendRatio * 100).toInt()}% of your work happens on weekends. Consider redistributing to weekdays.',
        priority: 'high',
    ...
    if (consistency < 0.6) {
      recommendations.add(Recommendation(
        ...
        description: 'Your work schedule varies significantly. Try establishing regular work blocks.',
        priority: 'medium',
        expectedImpact: '+25% consistency score',
      ));
    }

    return recommendations..sort((a, b) => _priorityValue(b.priority).compareTo(_priorityValue(a.priority)));
  }
  ...
  static ProductivityScore calculateProductivityScore(List<TimeEntry> entries) {
    if (entries.length < 3) {
      return ProductivityScore(
        ...
        balance: 50,
        efficiency: 50,
        insights: ['Need more data for accurate scoring (minimum 3 entries)'],
      );
    }
    ...
    final targetHours = 35.0;
    final volumeScore = min((stats.totalHours / targetHours) * 100, 100).toInt();
    if (volumeScore < 70) insights.add('Volume below target - worked ${stats.totalHours.toStringAsFixed(1)}h vs ${targetHours}h target');

    final consistencyScore = (_calculateConsistency(entries) * 100).toInt();
    if (consistencyScore < 70) insights.add('Inconsistent schedule - try establishing regular work blocks');

    final switchRate = _calculateSwitchRate(entries);
    final focusScore = ((1 - switchRate) * 100).toInt();
    if (focusScore < 70) insights.add('High task fragmentation - worked on ${stats.avgProjectsPerDay.toStringAsFixed(1)} projects/day');

    final weekendRatio = _calculateWeekendRatio(entries);
    final balanceScore = (max(0, 1 - weekendRatio * 2) * 100).toInt();
    if (balanceScore < 70) insights.add('Work-life balance needs attention - ${(weekendRatio * 100).toInt()}% weekend work');

    final avgSession = _calculateAvgSessionLength(entries);
    final efficiencyScore = avgSession > 2 && avgSession < 4 ? 90 : (avgSession > 4 ? 60 : 70);
    if (efficiencyScore < 70) insights.add('Session length suboptimal - average ${avgSession.toStringAsFixed(1)}h per session');

    final overall = ((volumeScore + consistencyScore + focusScore + balanceScore + efficiencyScore) / 5).round();

    if (insights.isEmpty) insights.add('Great work! All productivity metrics are healthy');
  ...
  static _Stats _computeStats(List<TimeEntry> entries) {
    ...
    final avgProjectsPerDay = (projectsPerDay.values.fold(0, (sum, s) => sum + s.length) / max(projectsPerDay.length, 1)).toDouble();
  ...
  static Map<_Pair<String, String>, int> _findTaskSequences(List<TimeEntry> entries) {
  ...
  static Pattern? _analyzeBreakPattern(List<TimeEntry> entries) {
    final sorted = entries.where((e) => e.startTime != null && e.endTime != null).toList()
    ...
    for (final e in sorted) {
      if (e.totalTime > 4) longSessions++;
    }

    if (longSessions > sorted.length * 0.5) {
      return Pattern(
        type: PatternType.breaks,
        description: 'You frequently work ${longSessions} long sessions (>4h) without breaks',
        confidence: 0.8,
        impact: 'negative',
      );
    }
  ...
  static Pattern? _analyzeTimeOfDay(List<TimeEntry> entries) {
    ...
    final morningHours = withTime.where((e) => e.startTime!.hour >= 6 && e.startTime!.hour < 12).fold(0.0, (sum, e) => sum + e.totalTime);
    final afternoonHours = withTime.where((e) => e.startTime!.hour >= 12 && e.startTime!.hour < 18).fold(0.0, (sum, e) => sum + e.totalTime);
    final eveningHours = withTime.where((e) => e.startTime!.hour >= 18 && e.startTime!.hour < 24).fold(0.0, (sum, e) => sum + e.totalTime);
    ...
    if (morningHours / total > 0.5) {
      return Pattern(
        ...
        description: 'You\'re a morning person - ${(morningHours / total * 100).toInt()}% of work happens 6am-12pm',
        confidence: 0.85,
        impact: 'positive',
      );
      ...
      return Pattern(
        type: PatternType.timeOfDay,
        description: 'You work best in evenings - ${(eveningHours / total * 100).toInt()}% of work happens after 6pm',
        confidence: 0.85,
        impact: 'neutral',
      );
    }
  ...
  static List<Recommendation> _analyzeOptimalTimes(List<TimeEntry> entries) {
    ...
    for (final entry in projectTimes.entries) {
      ...
      final avgHour = (entry.value.reduce((a, b) => a + b) / entry.value.length).toDouble();
      final timeSlot = avgHour < 12 ? 'morning (${avgHour.toInt()}am)' : avgHour < 18 ? 'afternoon (${avgHour.toInt() - 12}pm)' : 'evening (${avgHour.toInt() - 12}pm)';
      
      recommendations.add(Recommendation(
        type: RecommendationType.scheduling,
        title: 'Optimal Time for ${entry.key}',
        description: 'You typically work on "${entry.key}" in the $timeSlot - schedule similar tasks then',
        priority: 'medium',
  ...
  static double _calculateAvgSessionLength(List<TimeEntry> entries) {
    final withTime = entries.where((e) => e.startTime != null && e.endTime != null);
  ...
  static double _calculateWeekendRatio(List<TimeEntry> entries) {
    final weekendHours = entries.where((e) => e.date.weekday >= 6).fold(0.0, (sum, e) => sum + e.totalTime);
    final totalHours = entries.fold(0.0, (sum, e) => sum + e.totalTime);
  ...
  static double _calculateConsistency(List<TimeEntry> entries) {
    ...
    final variance = values.fold(0.0, (sum, v) => sum + pow(v - avg, 2)) / values.length;
  ...
  static String _dayName(int weekday) => ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][weekday - 1];
}
...
class _Stats {
  ...
  _Stats({required this.totalHours, required this.avgDailyHours, required this.avgProjectsPerDay});
}
...
class _Pair<T1, T2> {
  ...
  @override
  bool operator ==(Object other) => other is _Pair && first == other.first && second == other.second;
...
class Anomaly {
  ...
  final DateTime date;
  Anomaly({required this.type, required this.severity, required this.message, required this.date});
}
...
class Pattern {
  ...
  final String impact;
  Pattern({required this.type, required this.description, required this.confidence, required this.impact});
}
...
class Recommendation {
  ...
  Recommendation({required this.type, required this.title, required this.description, required this.priority, required this.expectedImpact});
}
...
class ProductivityScore {
  ...
  ProductivityScore({required this.overall, required this.volume, required this.consistency, required this.focus, required this.balance, required this.efficiency, required this.insights});
}
